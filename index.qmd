...
# Calculate overall means for each doctor
dreamy_mean_sev = dreamy_data_sev['post_surgical_score'].mean()
duck_mean_sev = duck_data_sev['post_surgical_score'].mean()

# Create figure with professional styling (same as previous figures)
fig, ax = plt.subplots(figsize=(12, 7))

# Add shaded region highlighting severity between -1 and 1 (zorder=0 so it's behind everything)
ax.axvspan(-1, 1, alpha=0.15, color='gray', zorder=0)

# Plot scatter points for Doc Dreamy (circles, blue) - severity on x-axis
ax.scatter(dreamy_data_sev['severity'], dreamy_data_sev['post_surgical_score'], 
           marker='o', color='#4E79A7', s=60, alpha=0.7, 
           label='Doc Dreamy', zorder=3)

# Plot scatter points for Doc Duck (triangles, coral) - severity on x-axis
ax.scatter(duck_data_sev['severity'], duck_data_sev['post_surgical_score'], 
           marker='^', color='#E15759', s=60, alpha=0.7, 
           label='Doc Duck', zorder=3)

# Add horizontal dashed lines for overall means
ax.axhline(y=dreamy_mean_sev, color='#4E79A7', linestyle='--', linewidth=2, 
           alpha=0.8, zorder=2)
ax.axhline(y=duck_mean_sev, color='#E15759', linestyle='--', linewidth=2, 
           alpha=0.8, zorder=2)

# Add text annotations with shape symbols and mean values
# Position annotations at the top right of the plot
max_severity = patients_df['severity'].max()
max_score = patients_df['post_surgical_score'].max()
ax.text(max_severity * 0.95, dreamy_mean_sev, f'○ {dreamy_mean_sev:.2f}', 
        color='#4E79A7', fontsize=11, fontweight='bold',
        verticalalignment='center', horizontalalignment='right',
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', 
                 edgecolor='#4E79A7', alpha=0.8), zorder=4)

ax.text(max_severity * 0.95, duck_mean_sev, f'△ {duck_mean_sev:.2f}', 
        color='#E15759', fontsize=11, fontweight='bold',
        verticalalignment='center', horizontalalignment='right',
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', 
                 edgecolor='#E15759', alpha=0.8), zorder=4)

# Professional styling (consistent with previous figures)
